    ...
    if (w.config.yaxis[realIndex].labels.show) {
      for (let i = tickAmount; i >= 0; i--) {
        ...
        let label = graphics.drawText({
          ...
          isPlainText: false,
          cssClass: 'apexcharts-yaxis-label ' + yaxisdashboard.cssClass
        })
        if (i === tickAmount) {
          firstLabel = label
        }
    ...
    if (w.config.yaxis[realIndex].title.text !== undefined) {
      ...
      let elYAxisTitleText = graphics.drawText({
        ...
        cssClass:
          ...
          w.config.yaxis[realIndex].title.dashboard.cssClass
      })
    ...
    if (w.config.xaxis.labels.show) {
      for (let i = tl ? 0 : tickAmount; tl ? i < tl : i >= 0; tl ? i++ : i--) {
        ...
        let elTick = graphics.drawText({
          ...
          isPlainText: false,
          cssClass:
            'apexcharts-xaxis-label ' + w.config.xaxis.labels.dashboard.cssClass
        })
